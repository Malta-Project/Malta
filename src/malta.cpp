...
void Malta::montecarlo(double (*integrand)(double)) {
    ...
    for (int i = 0; i < this->N_points; i++) {
        ...
        x = this->interval_borders[interval_idx] + Math::get_random_point(this->interval_borders[interval_idx], this->interval_borders[interval_idx]+ this->dx_i[interval_idx]);
        y = integrand(x);
